...
public class MazeRunner extends Cell {
	...
	private ArrayList<Cell> solvedMaze;
	private Cell [][] maze;
...
//				setDirection(temp);
	...
	public void selectClosest(ArrayList<Cell> cells)
	{
		Cell endCell = maze[COLS-1][ROWS-1];
		Cell closest = cells.get(0);
		for(int x = 0;x<cells.size();x++){
			if(cells.get(x).distance(endCell)<closest.distance(endCell)){
				closest = cells.get(x);
			}
		}
		
		setDirection(closest);
	}
}
